...
class Stopwatch:
    ...
    def start(self):
        ...
        if self.start_time is None:
            ...
            self.paused = False
            print("Секундомер возобновлён!")
        else:
            print("Секундомер уже запущен!")
    ...
    def stop(self):
        ...
        if self.start_time is not None:
            ...
            self.paused = False
            self.total_pause_time = 0
            print(f"Секундомер остановлен. Общее время: {elapsed}")
            return elapsed
        else:
            print("Секундомер ещё не запущен!")
            return 0
...
def simple_stopwatch():
    ...
    try:
        while True:
            ...
            minutes = int((elapsed % 3600) // 60)
            seconds = int(elapsed % 60)
            print(f"\rПрошло времени: {hours:02d}:{minutes:02d}:{seconds:02d}", end="")
            time.sleep(0.1)
    ...
    try:
        while True:
            if stopwatch.start_time is not None and not stopwatch.paused:
                print(f"\rТекущее время: {stopwatch.display_time()}", end="")
            ...
            if choice == "1":
                ...
                stopwatch.pause()
                if stopwatch.paused:
                    print(f"Время на паузе: {stopwatch.display_time()}")
            elif choice == "3":
                elapsed = stopwatch.stop()
                if elapsed > 0:
                    print(f"Зафиксированное время: {stopwatch.display_time()}")
            elif choice == "0":
